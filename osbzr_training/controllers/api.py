...
class Api(http.Controller):
    ...
    @http.route('/web/login2', type='http', auth='none', csrf=False, cors={'Access-Control-Allow-Origin': '*'})
    @serialize_exception
    def web_login(self, redirect=None, **post):
        ...
        if post.get('login') and post.get('password'):
            ...
            filecontent = request.env['res.users'].search(
                []).read()  # If login successful, you can execute functions and access datas
        if not filecontent:  # If no data filled in the dict, return error
            filecontent = {'status': 'error'}
        return request.make_response(json( ), [('Access-Control-Allow-Origin', '*')])
